    ...
    if (body.includes("ataturk")) {
      ...
      if (currentTime - lastResponseTime >= cooldownPeriod) {
        ...
        const randomPhoto: string =
          ataturkPhotos[Math.floor(Math.random() * ataturkPhotos.length)];

        comment.reply(
          `Here's a random quote from Mustafa Kemal Ataturk:\n\n"${randomQuote}"\n\nRandom Ataturk Photo: ${randomPhoto}`
        );
...
async function startBot(): Promise<void> {
  ...
  subreddit.stream
    .comments()
    .on("comment", async (comment) => await processComments(comment));
}
// Start the bot
startBot();
